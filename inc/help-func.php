...
function it_posted_on() {
	...
	/* translators: %s: post date. */
		esc_html_x( 'Posted on %s', 'post date', 'parla' ), $time_string
	);
...
function it_posted_by() {
	...
	/* translators: %s: post author. */
		esc_html_x( 'by %s', 'post author', 'parla' ),
...
function it_cat_links() {
	$categories_list = get_the_category_list( esc_html__( ', ', 'parla' ) );
	if ( $categories_list ) {
		printf( '<div class="cat-links">' . esc_html__( 'Posted in %1$s', 'parla' ) . '</div>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
...
function it_tag_links() {
	...
	if ( $tags_list ) {
		printf( '<div class="tag-links">' . esc_html__( 'Tagged %1$s', 'parla' ) . '</div>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
...
function it_inline_svg( $svg_url ) {
	...
	if ( $extension == 'svg' ){ // Also I'd add some extra security check for mime type
...
if ( ! function_exists( 'it_log' ) ) {
	 ...
	 * @param $file - log file location (website root folder by default)
	 *
	 */
	function it_log( $var, $label = null, $file = ABSPATH . '/my.log' ) {
